class Obstacle {
  constructor(ctx, damage, obstacleImg, obstacPlacing) {
    ...
    this.y = obstacPlacing || Math.random() * this.ctx.canvas.height;
  ...
  collides(objetivo) {
    const colX =
      this.x <= objetivo.x + objetivo.w - 5 && this.x + this.w > objetivo.x;
    const colY =
      this.y + this.h - 5 > objetivo.y && this.y < objetivo.y + objetivo.h - 5;
    return colX && colY;
  }
}
